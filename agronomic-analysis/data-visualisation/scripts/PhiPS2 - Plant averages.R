...
# Read the data
data <- read.csv("Masters/data/salt_stress/porometer_excl_dead/PhiPS2 - Plant averages.csv", header=TRUE, stringsAsFactors=FALSE, check.names = FALSE, sep = ",")
...
write.csv(right_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/porometer_excl_dead/PhiPS2_plantAverage_right_tailed_results.csv", row.names = FALSE)

left_tailed_results  <- map_dfr(all_weeks, perform_tests_for_week, direction = "left")
write.csv(left_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/porometer_excl_dead/PhiPS2_plantAverages_left_tailed_results.csv", row.names = FALSE)
...
print(p2)
ggsave(filename = paste0(output_dir, "PhiPS2_plantAverages_scatter_plot_exact.png"), plot = p2, width = 8, height = 6, dpi = 300)
...
print(p3)
ggsave(filename = paste0(output_dir, "PhiPS2_plantAverages_scatter_plot_jittered.png"), plot = p3, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (without jitter)
print(p2_sig)
ggsave(filename = paste0(output_dir, "PhiPS2_plantAverages_scatter_plot_exact.png"), plot = p2_sig, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (with jitter)
print(p3_sig)
ggsave(filename = paste0(output_dir, "PhiPS2_plantAverages_scatter_plot_jittered.png"), plot = p3_sig, width = 8, height = 6, dpi = 300)
...
pvalue_heatmap <- ggplot(RT_Students_t_test_results, aes(x = Week, y = 1, fill = p_value)) +
  ...
  ggtitle("Right-tailed Student's t-test p-values by Week for PhiPS2 (plant averages)")

# Print the plot
print(pvalue_heatmap)
ggsave(filename = paste0(output_dir, "PhiPS2_plant_averages.png"), plot = pvalue_heatmap, width = 8, height = 6, dpi = 300)