...
# Read the data
data <- read.csv("Masters/data/salt_stress/all_incl_dead/VPDLeaf - Individual readings.csv", header=TRUE, stringsAsFactors=FALSE, check.names = FALSE, sep = ",")
...
write.csv(right_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/all_incl_dead/VPDLeaf_individualReadings_right_tailed_results.csv", row.names = FALSE)

left_tailed_results  <- map_dfr(all_weeks, perform_tests_for_week, direction = "left")
write.csv(left_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/all_incl_dead/VPDLeaf_individualReadings_left_tailed_results.csv", row.names = FALSE)
...
# Print and save the scatter plot (without jitter)
print(p2_sig)
ggsave(filename = paste0(output_dir, "VPDLeaf_individualReadings_scatter_plot_exact.png"), plot = p2_sig, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (with jitter)
print(p3_sig)
ggsave(filename = paste0(output_dir, "VPDLeaf_individualReadings_scatter_plot_jittered.png"), plot = p3_sig, width = 8, height = 6, dpi = 300)
...
pvalue_heatmap <- ggplot(RT_Students_t_test_results, aes(x = Week, y = 1, fill = p_value)) +
  ...
  ggtitle("Right-tailed Student's t-test p-values by Week for VPDLeaf (individual readings)")

# Print the plot
print(pvalue_heatmap)
ggsave(filename = paste0(output_dir, "VPDLeaf_individual_readings.png"), plot = pvalue_heatmap, width = 8, height = 6, dpi = 300)