...
# Read the data
data <- read.csv("Masters/data/salt_stress/all_incl_dead/PhiPS2 - Individual readings.csv", header=TRUE, stringsAsFactors=FALSE, check.names = FALSE, sep = ",")
...
write.csv(right_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/all_incl_dead/PhiPS2_individualReadings_right_tailed_results.csv", row.names = FALSE)

left_tailed_results  <- map_dfr(all_weeks, perform_tests_for_week, direction = "left")
write.csv(left_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/all_incl_dead/PhiPS2_individualReadings_left_tailed_results.csv", row.names = FALSE)

# Create the scatter plot (without jitter)
p2 <- ggplot(data_long, aes(x = Week, y = Value, color = Treatment)) +
  geom_point(size = 3, alpha = 0.7) +
  labs(title = "Weekly PhiPS2 per Treatment",
       x = "Week",
       y = "PhiPS2",
       color = "Treatment") +
  theme_minimal() +
  ...
  annotate("text", x = 1, y = 0, label = "T1", vjust = 2, hjust = -0.5, angle = 0, size = 3) +
  annotate("text", x = 5, y = 0, label = "T2", vjust = 2, hjust = -0.5, angle = 0, size = 3) +
  annotate("text", x = 9, y = 0, label = "T3", vjust = 2, hjust = -0.5, angle = 0, size = 3)

# Print and save the scatter plot (without jitter)
print(p2)
ggsave(filename = paste0(output_dir, "PhiPS2_individualReadings_scatter_plot_exact.png"), plot = p2, width = 8, height = 6, dpi = 300)
...
print(p3)
ggsave(filename = paste0(output_dir, "PhiPS2_individualReadings_scatter_plot_jittered.png"), plot = p3, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (without jitter)
print(p2_sig)
ggsave(filename = paste0(output_dir, "PhiPS2_individualReadings_scatter_plot_exact.png"), plot = p2_sig, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (with jitter)
print(p3_sig)
ggsave(filename = paste0(output_dir, "PhiPS2_individualReadings_scatter_plot_jittered.png"), plot = p3_sig, width = 8, height = 6, dpi = 300)
...
pvalue_heatmap <- ggplot(RT_Students_t_test_results, aes(x = Week, y = 1, fill = p_value)) +
  ...
  ggtitle("Right-tailed Student's t-test p-values by Week for PhiPS2 (individual readings)")

# Print the plot
print(pvalue_heatmap)
ggsave(filename = paste0(output_dir, "PhiPS2_individual_readings.png"), plot = pvalue_heatmap, width = 8, height = 6, dpi = 300)