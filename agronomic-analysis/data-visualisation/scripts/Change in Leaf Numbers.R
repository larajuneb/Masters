...
# Read the data
data <- read.csv("Masters/data/salt_stress/all_incl_dead/Change in Leaf Numbers.csv", header=TRUE, stringsAsFactors=FALSE, check.names = FALSE)
...
write.csv(right_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/all_incl_dead/ChangeInLeafNumber_right_tailed_results.csv", row.names = FALSE)

left_tailed_results  <- map_dfr(all_weeks, perform_tests_for_week, direction = "left")
write.csv(left_tailed_results, "Masters/data_visualization/agronomic/test_results/salt_stress/all_incl_dead/ChangeInLeafNumber_left_tailed_results.csv", row.names = FALSE)
...
# Create a bar plot
p_bar_sig <- ggplot(data_long, aes(x = Week, y = Value, fill = Treatment)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Weekly Change in Leaf Number per Treatment",
       x = "Week",
       y = "Leaves Gained",
...
# Save the bar plot as a PNG file
ggsave(filename = paste0(output_dir, "ChangeInLeafNumbers_bar_plot.png"), plot = p_bar_sig, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (without jitter)
print(p2_sig)
ggsave(filename = paste0(output_dir, "ChangeInLeafNumbers_scatter_plot_exact.png"), plot = p2_sig, width = 8, height = 6, dpi = 300)
...
# Print and save the scatter plot (with jitter)
print(p3_sig)
ggsave(filename = paste0(output_dir, "ChangeInLeafNumbers_scatter_plot_jittered.png"), plot = p3_sig, width = 8, height = 6, dpi = 300)
...
output_row <- c(DataType = "Change in Leaf Number", setNames(as.list(ranked_weeks$Week), as.character(ranked_weeks$Rank)))
